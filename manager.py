...
class TourManager:
    ...
    async def run(self, query: str, interests: list, duration: str) -> str:
        try:
            return await self._run_logic(query, interests, duration)
        except openai.AuthenticationError:
            self.printer.end()
            raise Exception("Authentication Error: The API key provided is invalid. Please check your .env file or sidebar input. Note that this project requires an OpenAI API key (starting with 'sk-'), but the current key looks like a Google Gemini key.")
        except Exception as e:
            self.printer.end()
            raise Exception(f"Tour Generation Failed: {str(e)}")

    async def _run_logic(self, query: str, interests: list, duration: str) -> str:
